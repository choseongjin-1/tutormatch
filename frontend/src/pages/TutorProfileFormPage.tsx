  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-6 text-2xl font-semibold text-gray-900">
        {profileQuery.data ? '내 프로필 수정' : '튜터 프로필 등록'}
      </h1>
...
function SlotManager({ tutorId }: { tutorId: number }) {
  ...
  return (
    <div className="mt-10 border-t border-gray-200 pt-8">
      <h2 className="mb-4 text-lg font-semibold text-gray-900">예약 가능 시간 관리</h2>
      <form onSubmit={handleAddSlot} className="mb-6 flex flex-wrap items-end gap-3">
        <div>
          <label htmlFor="slotDate" className="mb-1 block text-sm font-medium text-gray-700">
            날짜
          </label>
          <input
            id="slotDate"
      ...
      {createSlotMutation.isError && (
        <p className="mb-4 text-sm text-red-600">
          {getApiErrorMessage(createSlotMutation.error, '슬롯 등록에 실패했습니다.')}
        </p>
      )}

      {slotsQuery.data && slotsQuery.data.length === 0 && (
        <p className="text-sm text-gray-500">등록된 시간이 없습니다.</p>
      )}
      {slotsQuery.data && slotsQuery.data.length > 0 && (
          ...
          {slotsQuery.data.map((slot) => (
            ...
            >
              <span className="text-gray-700">
                ...
                {slot.booked && <span className="ml-2 text-xs text-gray-500">(예약됨)</span>}
              </span>
              <button
                type="button"
